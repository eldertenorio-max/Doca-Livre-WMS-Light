...
/**
 * Lista oficial enviada (1ª–4ª contagem): descrição e unidade exibidas na checklist armazém,
 * independentemente do texto em "Todos os Produtos".
 * Ordem dos códigos = `listArmazemContagemCodigosOrdered` / armazemInventarioMap.
 */
const ARMAZEM_LISTA_OFICIAL: ReadonlyArray<readonly [codigo: string, descricao: string, unidade: string]> = [
...
const byCodigo = new Map<string, { descricao: string; unidade: string }>()
for (const [codigo, descricao, unidade] of ARMAZEM_LISTA_OFICIAL) {
  byCodigo.set(codigo, { descricao, unidade })
}

/** Metadados da planilha oficial (88 itens). */
export function getArmazemListaOficialMeta(codigo: string): { descricao: string; unidade: string } | undefined {
  const t = String(codigo ?? '').trim()
  let m = byCodigo.get(t)
  if (m) return m
  const n = normalizeCodigoInternoCompareKey(t)
  if (!n) return undefined
  for (const [c, meta] of byCodigo) {
    if (normalizeCodigoInternoCompareKey(c) === n) return meta
  }
  return undefined
}

export const ARMAZEM_LISTA_OFICIAL_COUNT = ARMAZEM_LISTA_OFICIAL.length
